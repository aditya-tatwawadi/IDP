...
void Stage2() {
  ...
  // Now go to D and face west
  InitialMovement();
  LineFollow(); //Now at D, facing west

  delay(1000);
  //Take another distance
  BlDist = sensor.readRangeContinuousMillimeters();
  // Checks if there is a block at B
  if (BlDist <= 600) {
    //TOFsensor.stop();
    InitialMovement();
    delay(1000); // TO BE DELETED LATER - FOR TESTING
    ...
    delay(1000);
    Rotate90AC(); // At D facing West

  }
  Rotate180(); //At D facing east
  delay(1000);
